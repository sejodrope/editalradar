...
def render(db: Session, perfil_id: Optional[int] = None) -> None:
    ...
    # Mostra apenas adequados para solo, não vencidos, ordenados por relevância
    from datetime import datetime as _dt
    _hoje = _dt.now()
    todos_interessantes = crud.listar_editais(
        db,
        perfil_id=perfil_id,
    ...
    interessantes = [
        e for e in todos_interessantes
        if (e.data_encerramento is None or e.data_encerramento.replace(tzinfo=None) >= _hoje)
